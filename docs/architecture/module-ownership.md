...
## Settings Hub additions (2026-05-03)

| Cluster | New owned models / fields | Notes |
|---------|--------------------------|-------|
| `platform/settings` | `PlatformSetting` (key, value, isSecret) | Read/write via `PlatformSettingsService.get(key, envFallback)` |
| `platform/email` | `PlatformEmailTemplate` (phase 1), `PlatformEmailLog` (phase 1) | Resend + platform SMTP; secrets encrypted via `encryptSecret/decryptSecret` |
| `platform/notifications-config` | settings keys only (`notifications.default.*`) | No Prisma model; all stored in `PlatformSetting` |
| `platform/system-health` | no Prisma models | Redis/Postgres/MinIO probes; top-consumers via `UsageCounter.groupBy` |
| `platform/branding` | settings keys only (`branding.adminLogoUrl`, `branding.adminPrimaryColor`) | No Prisma model; served to admin shell at runtime |
| `identity/two-factor` | 3 new columns on `User` (`twoFactorSecret`, `twoFactorEnabledAt`, `twoFactorRecoveryHash`); new model `FailedLoginAttempt` | TOTP via `otplib`; recovery hash = sha256; single-use |
| `comms` | `NotificationDeliveryLog.senderActor` (enum `NotificationSenderActor`) | New field added in phase 6 migration `add_sender_actor_to_delivery_log` |

## Security hardening (2026-05-03)

| Feature | Location | Behavior |
|---------|----------|----------|
| Super-admin session TTL | `SuperAdminGuard` + setting `security.session.superAdminTtlMinutes` | Rejects JWTs older than configured TTL (default 60 min) |
| IP allowlist | `SuperAdminIpAllowlistMiddleware` + setting `security.ipAllowlist` | Blocks `/admin/*` from non-listed CIDRs when list is non-empty |
| TOTP 2FA | `identity/two-factor` slices + login handler gate | Optional per-account or required globally via `security.twoFactor.required` |
| Failed-login log | `FailedLoginAttempt` model | Written on every failed login branch; readable via `GET /admin/settings/security/failed-logins` |
| Emergency recovery | `docs/operations/super-admin-2fa-recovery.md` | psql + Prisma Studio procedure; includes policy-disable escape hatch |
