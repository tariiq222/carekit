...
export class GroupsPaymentService {
  ...
  async triggerPaymentRequest(groupId: string): Promise<{
    ...
    if (group.status !== 'open' && group.status !== 'full') {
      throw new BadRequestException(
        'Group must be open or full to trigger payment request',
      );
    }

    if (group.currentEnrollment < group.minParticipants) {
      throw new BadRequestException(
        'Group does not meet minimum participant requirement',
      );
    }

    const deadlineAt = new Date(
      Date.now() + group.paymentDeadlineHours * 60 * 60 * 1000,
    );
    ...
    return {
      triggered: true,
      enrollmentsNotified: enrollments.length,
      deadlineAt,
    };
  }
  ...
  async resendPaymentRequest(
    ...
    if (!enrollment) {
      throw new NotFoundException(
        'Enrollment not found or not awaiting payment',
      );
    }

    const requiredAmountHalalat = this.getRequiredAmount(
      enrollment.group as {
