...
describe('ApplyCouponHandler', () => {
  it('applies percentage coupon and returns redemption', async () => {
    ...
    const handler = new ApplyCouponHandler(prisma as never, buildTenant() as never, buildFeatureCheck() as never);
  ...
  it('throws NotFoundException when invoice not found', async () => {
    ...
    await expect(new ApplyCouponHandler(prisma as never, buildTenant() as never, buildFeatureCheck() as never).execute(cmd)).rejects.toThrow(NotFoundException);
  });
  ...
  it('throws BadRequestException when coupon expired', async () => {
    ...
    await expect(new ApplyCouponHandler(prisma as never, buildTenant() as never, buildFeatureCheck() as never).execute(cmd)).rejects.toThrow(BadRequestException);
  });
  ...
  it('throws BadRequestException when COUPONS feature is disabled', async () => {
    const prisma = buildPrisma();
    await expect(
      new ApplyCouponHandler(prisma as never, buildTenant() as never, buildFeatureCheck(false) as never).execute(cmd),
