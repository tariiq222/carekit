...
import { MoyasarWebhookHandler } from './moyasar-webhook.handler';
...
describe('MoyasarWebhookHandler', () => {
  describe('verifySignature', () => {
    it('passes for valid signature', () => {
      const handler = new MoyasarWebhookHandler(buildPrisma() as never, buildEventBus() as never);
      const body = '{"id":"test"}';
  ...
  describe('execute', () => {
    it('processes paid webhook and emits PaymentCompletedEvent', async () => {
      ...
      const handler = new MoyasarWebhookHandler(prisma as never, eventBus as never);

      const result = await handler.execute(paidPayload);
    ...
    it('skips when metadata is missing', async () => {
      const handler = new MoyasarWebhookHandler(buildPrisma() as never, buildEventBus() as never);
      const result = await handler.execute({ ...paidPayload, metadata: undefined });
      expect(result.skipped).toBe(true);
    });

    it('creates failed payment for non-paid status', async () => {
      const prisma = buildPrisma();
      const handler = new MoyasarWebhookHandler(prisma as never, buildEventBus() as never);

      await handler.execute({ ...paidPayload, status: 'failed', message: 'Declined' });
