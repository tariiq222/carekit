...
describe('CustomDomainGraceCron', () => {
  it('sends warning email when daysLeft <= 7 (> 0)', async () => {
    ...
    const cron = new CustomDomainGraceCron(prisma as never, mailer as never, buildConfig() as never, buildCls() as never);
    await cron.run();
  ...
  it('skips when BILLING_CRON_ENABLED is false', async () => {
    ...
    const cron = new CustomDomainGraceCron(prisma as never, mailer as never, buildConfig(false) as never, buildCls() as never);
    await cron.run();
  ...
  it('wraps run body in super-admin CLS context', async () => {
    ...
    const prisma = buildPrisma(makeOrg(5));
    const cls = buildCls();
    const cron = new CustomDomainGraceCron(prisma as never, mailer as never, buildConfig() as never, cls as never);
