...
describe('encodeZatcaQr', () => {
  ...
  it('encodes Arabic seller names via UTF-8', () => {
    const b64 = encodeZatcaQr({ ...sample, sellerName: 'منصة كير كِت' });
    const parts = decodeTlv(b64);
    expect(parts[0]).toEqual({ tag: 1, value: 'منصة كير كِت' });
  });
  ...
  it('emits a known-vector base64 for a fixed input', () => {
    ...
    const expected = Buffer.concat([
      Buffer.from([1, 16]),
      Buffer.from('CareKit Platform', 'utf8'),
      Buffer.from([2, 15]),
