...
export class BookingRescheduleService {
  ...
  async reschedule(
    ...
    const [ps, svc, settings] = await Promise.all([
      this.prisma.practitionerService.findUnique({
        where: { id: booking.practitionerServiceId },
      }),
      this.prisma.service.findFirst({ where: { id: booking.serviceId } }),
      this.settingsService.getForBranch(booking.branchId ?? undefined),
    ]);
    const duration =
      booking.bookedDuration ?? ps?.customDuration ?? svc?.duration ?? 30;
    ...
    if (booking.type === 'online') {
      ...
      const mtg = await this.zoomService.createMeeting(
        'CareKit Video Consultation',
        isoStart,
        duration,
      );
      zoomData = {
        zoomMeetingId: mtg.meetingId,
        zoomJoinUrl: mtg.joinUrl,
        zoomHostUrl: mtg.hostUrl,
      };
    }
      ...
      async (tx) => {
        ...
        await tx.booking.update({
          ...
          data: {
            ...
            adminNotes: `Rescheduled to booking ${nb.id}`,
          },
        });
        await tx.payment.updateMany({
          where: { bookingId: id },
          data: { bookingId: nb.id },
        });
        return nb;
      },
    ...
    if (booking.type === 'online' && oldZoomId) {
      this.zoomService
        .deleteMeeting(oldZoomId)
        .catch((e) =>
          this.logger.warn(`Failed to delete old Zoom meeting: ${e.message}`),
        );
    }
