...
describe('RescheduleBookingHandler', () => {
  ...
  it('reschedules booking when new slot is free', async () => {
    ...
    await new RescheduleBookingHandler(prisma as never, buildTenant() as never, defaultRescheduleSettings as never, { updateMeeting: jest.fn().mockResolvedValue(undefined) } as never).execute({
  ...
  it('throws BadRequestException when booking is COMPLETED', async () => {
    ...
    await expect(
      new RescheduleBookingHandler(prisma as never, buildTenant() as never, defaultRescheduleSettings as never, { updateMeeting: jest.fn().mockResolvedValue(undefined) } as never).execute({
        bookingId: 'book-1', newScheduledAt: newFuture, changedBy: 'user-42',
      }),
...
describe('RescheduleBookingHandler — maxReschedulesPerBooking', () => {
  it('allows reschedule when count is below max', async () => {
    ...
    const handler = new RescheduleBookingHandler(prisma as never, buildTenant() as never, settingsHandler as never, { updateMeeting: jest.fn().mockResolvedValue(undefined) } as never);
