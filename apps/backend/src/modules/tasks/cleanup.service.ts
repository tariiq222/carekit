...
export class CleanupService {
  ...
  async cleanOldProcessedWebhooks() {
    ...
    if (result.count > 0) {
      this.logger.log(
        `Cleaned ${result.count} processed webhooks older than ${WEBHOOK_RETAIN_DAYS} days`,
      );
    }
  }
  ...
  async archiveOldActivityLogs() {
    ...
    try {
      ...
      if (result > 0) {
        this.logger.log(
          `Archived ${result} activity logs older than ${ACTIVITY_ARCHIVE_DAYS} days`,
        );
      }
    } catch (err) {
      ...
      if (
        msg.includes('activity_logs_archive') &&
  ...
  async logTableGrowthSnapshot() {
    ...
    this.metricsService.dbTableRows.set(
      { table: 'activity_logs' },
      activityLogs,
    );
    this.metricsService.dbTableRows.set(
