...
describe('Branch-Aware Booking (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!categoryId)
      throw new Error(
        `Category creation failed: ${JSON.stringify(catRes.body)}`,
      );
    ...
    const svcRes = await request(httpServer)
      ...
      .set(getAuthHeaders(superAdmin.accessToken))
      .send({
        nameEn: 'Branch Test Service',
        nameAr: 'خدمة اختبار الفرع',
        categoryId,
        price: 10000,
        duration: 30,
      });
    ...
    if (!serviceId)
      throw new Error(
        `Service creation failed: ${JSON.stringify(svcRes.body)}`,
      );
  ...
  describe('POST /bookings with branchId', () => {
    ...
    it('should return 400 PRACTITIONER_BRANCH_MISMATCH when practitioner not assigned to branch', async () => {
      const res = await request(httpServer)
        ...
        .set(getAuthHeaders(superAdmin.accessToken))
        .send({
          practitionerId: unassignedPractitionerId,
          serviceId,
          branchId,
          date: bookingDate,
          startTime: '10:00',
  ...
  describe('Multi-branch availability — global availability fallback', () => {
    ...
    beforeAll(async () => {
      ...
      const pUser = await createTestUserWithRole(
        ...
        {
          email: 'multi.branch.prac@carekit-test.com',
          password: 'MultiPrac@123',
          firstName: 'فارس',
          lastName: 'المتعدد',
          phone: '+966599000001',
          gender: 'male',
      ...
      const pRes = await request(httpServer)
        ...
        .set(getAuthHeaders(superAdmin.accessToken))
        .send({
          userId: pUser.user.id,
          specialtyId: await getFirstSpecialtyId(
            httpServer,
            superAdmin.accessToken,
          ),
        })
      ...
      await request(httpServer)
        ...
        .set(getAuthHeaders(superAdmin.accessToken))
        .send({ practitionerIds: [multiBranchPractitionerId] })
        .expect(200);
      await request(httpServer)
      ...
      await request(httpServer)
        ...
        .send({
          schedule: [
            {
              dayOfWeek: dayAfterTomorrow.getDay(),
              startTime: '08:00',
              endTime: '16:00',
              isActive: true,
            },
      ...
      const catId2 =
        (catRes2.body.data as { id: string } | undefined)?.id ?? '';

      const svcRes2 = await request(httpServer)
        .post(`${API_PREFIX}/services`)
        .set(getAuthHeaders(superAdmin.accessToken))
        .send({
          nameEn: 'Multi Branch Svc',
          nameAr: 'خدمة متعدد',
          categoryId: catId2,
          price: 10000,
          duration: 30,
        });
      multiBranchServiceId =
        (svcRes2.body.data as { id: string } | undefined)?.id ?? '';
      ...
      await request(httpServer)
        ...
        .set(getAuthHeaders(superAdmin.accessToken))
        .send({
          serviceId: multiBranchServiceId,
    ...
    it('should allow booking at branch A with global availability', async () => {
      const res = await request(httpServer)
        ...
        .set(getAuthHeaders(superAdmin.accessToken))
        .send({
          practitionerId: multiBranchPractitionerId,
          serviceId: multiBranchServiceId,
          branchId: branchAId,
          date: multiBranchDate,
          startTime: '08:00',
          type: 'in_person',
          patientId: patient.user.id as string,
        })
...
async function getFirstSpecialtyId(
  httpServer: unknown,
  token: string,
): Promise<string> {
  const res = await request(httpServer as Parameters<typeof request>[0])
    ...
    .expect(200);
  const specialties = (res.body.data?.items ?? res.body.data) as Array<{
    id: string;
  }>;
