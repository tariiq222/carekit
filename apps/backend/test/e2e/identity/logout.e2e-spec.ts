...
describe('POST /auth/logout (e2e)', () => {
  ...
  it('✅ logout يحذف الـ refresh token من DB', async () => {
    const before = await (testPrisma as any).refreshToken.count({ where: { tenantId: TENANT } });
    ...
    const after = await (testPrisma as any).refreshToken.count({ where: { tenantId: TENANT, revokedAt: null } });
    expect(after).toBe(0);
  });
