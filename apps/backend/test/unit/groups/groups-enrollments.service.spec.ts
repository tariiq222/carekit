...
describe('GroupsEnrollmentsService', () => {
  ...
  describe('enroll()', () => {
    it('throws 404 when group not found', async () => {
      ...
      await expect(service.enroll('grp-x', 'pat-1')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('throws 400 when enrolling in a full group', async () => {
      ...
      await expect(service.enroll('grp-1', 'pat-1')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should not allow overbooking when two enrollments race', async () => {
      ...
      const mockTx = {
        ...
        group: {
          findFirst: jest
            .fn()
            .mockResolvedValue({ ...group, currentEnrollment: 9 }),
  ...
  describe('cancelEnrollment()', () => {
    it('throws 404 when enrollment not found', async () => {
      mockPrisma.groupEnrollment.findFirst.mockResolvedValue(null);
      await expect(service.cancelEnrollment('enr-x', 'pat-1')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('throws 400 when enrollment is not in registered status', async () => {
      ...
      await expect(service.cancelEnrollment('enr-1', 'pat-1')).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('removeEnrollment()', () => {
    it('throws 404 when enrollment not found', async () => {
      mockPrisma.groupEnrollment.findFirst.mockResolvedValue(null);
      await expect(service.removeEnrollment('enr-x')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('throws 400 when enrollment is confirmed', async () => {
      ...
      await expect(service.removeEnrollment('enr-1')).rejects.toThrow(
        BadRequestException,
      );
    });
