...
const mockInvoicesServiceDefault = {
  createInvoice: jest.fn().mockResolvedValue({ id: 'inv-default' }),
};
...
describe('PaymentsService — createMoyasarPayment', () => {
  ...
  it('should delegate to MoyasarPaymentService and return payment + redirectUrl', async () => {
    ...
    expect(mockMoyasar.createMoyasarPayment).toHaveBeenCalledWith(
      mockUserId,
      createMoyasarDto,
    );
  ...
  it.each([
    ...
    'should propagate %s from MoyasarPaymentService',
    async (_label, Exception, message) => {
      mockMoyasar.createMoyasarPayment.mockRejectedValue(
...
describe('PaymentsService — handleMoyasarWebhook', () => {
  ...
  it('should delegate to MoyasarPaymentService', async () => {
    ...
    expect(mockMoyasar.handleMoyasarWebhook).toHaveBeenCalledWith(
      'valid-sig',
      rawBody,
      webhookDto,
    );
  ...
  it('should propagate errors from MoyasarPaymentService', async () => {
    const rawBody = Buffer.from(JSON.stringify(webhookDto));
    mockMoyasar.handleMoyasarWebhook.mockRejectedValue(
...
describe('PaymentsService — updateStatus invoice creation', () => {
  ...
  it('should swallow ConflictException if invoice already exists', async () => {
    ...
    await expect(
      service.updateStatus(mockPaymentId, {
        status: 'paid',
        transactionRef: 'TXN-123',
      }),
