    ...
    const rowA = await h.runAs({ organizationId: a.id }, () =>
      h.prisma.organizationPaymentConfig.create({
        data: {
          ...
          webhookSecretEnc: enc.encrypt({ webhookSecret: 'wh_secret_aaaa' }, a.id),
          isLive: false,
        },
    ...
    const rowB = await h.runAs({ organizationId: b.id }, () =>
      h.prisma.organizationPaymentConfig.create({
        data: {
          ...
          webhookSecretEnc: enc.encrypt({ webhookSecret: 'wh_secret_bbbb' }, b.id),
          isLive: false,
        },
        select: { id: true, organizationId: true },
      }),
    ...
    await h.runAs({ organizationId: a.id }, () =>
      h.prisma.organizationPaymentConfig.create({
        data: {
          ...
          webhookSecretEnc: enc.encrypt({ webhookSecret: 'wh_secret_rls' }, a.id),
          isLive: false,
        },
      }),
    );
