...
- P54 (أصلي): breaks GET يتطلب `practitioners:view` (ليس عاماً) — تحديد الـ auth مهم

---

## إنشاء طبيب — `POST /practitioners`

> يتطلب صلاحية `practitioners:create`

| # | الاسم | الوصف | النتيجة المتوقعة |
|---|-------|-------|-----------------|
| PR-C1 | إنشاء أساسي | userId + specialty | 201 + practitioner.id |
...
| PR-C9 | بدون صلاحية | مستخدم بدون practitioners:create | 403 FORBIDDEN |
| PR-C10 | بدون مصادقة | POST /practitioners بدون token | 401 AUTH_TOKEN_INVALID |

---

## الإعداد السريع — `POST /practitioners/onboard`

> يتطلب صلاحية `practitioners:create` — يُنشئ مستخدماً وطبيباً في خطوة واحدة

| # | الاسم | الوصف | النتيجة المتوقعة |
|---|-------|-------|-----------------|
| O1 | إعداد كامل | nameEn + nameAr + email + specialty | 200 + { success: true, practitioner } |
| O2 | مع avatarUrl | تضمين رابط صورة صالح | 200 + practitioner.user.avatarUrl محفوظ |
...
| O10 | بدون صلاحية | مستخدم بدون practitioners:create | 403 FORBIDDEN |
| O11 | بدون مصادقة | POST /practitioners/onboard بدون token | 401 AUTH_TOKEN_INVALID |

---

## قراءة الأطباء — `GET /practitioners` (عام، بدون JWT)

| # | الاسم | الوصف | النتيجة المتوقعة |
|---|-------|-------|-----------------|
| PR-L1 | قائمة بدون توكن | GET /practitioners | 200 + مصفوفة مع pagination |
...
| PR-L12 | تفاصيل طبيب | GET /practitioners/:id | 200 + ملف كامل مع services + ratings |
| PR-L13 | طبيب وهمي | GET /practitioners/:fakeUUID | 404 PRACTITIONER_NOT_FOUND |

---

## تعديل الطبيب — `PATCH /practitioners/:id`

> يتطلب صلاحية `practitioners:edit` — الطبيب يستطيع تعديل ملفه فقط
...
| PR-U8 | طبيب يعدّل ملف طبيب آخر | practitionerId مختلف | 403 FORBIDDEN |
| PR-U9 | بدون صلاحية | مستخدم بدون practitioners:edit | 403 FORBIDDEN |
| PR-U10 | طبيب وهمي | UUID وهمي | 404 PRACTITIONER_NOT_FOUND |

---

## حذف الطبيب — `DELETE /practitioners/:id`

> يتطلب صلاحية `practitioners:delete` — soft delete

| # | الاسم | الوصف | النتيجة المتوقعة |
|---|-------|-------|-----------------|
| PR-D1 | حذف ناجح | المدير يحذف طبيباً | 200 + يختفي من GET /practitioners |
| PR-D2 | GET بعد الحذف | GET /practitioners/:id المحذوف | 404 PRACTITIONER_NOT_FOUND |
| PR-D3 | حذف مرتين | UUID لطبيب محذوف مسبقاً | 404 PRACTITIONER_NOT_FOUND |
| PR-D4 | طبيب وهمي | UUID غير موجود | 404 PRACTITIONER_NOT_FOUND |
| PR-D5 | بدون صلاحية | مستخدم بدون practitioners:delete | 403 FORBIDDEN |
| PR-D6 | بدون مصادقة | DELETE بدون token | 401 AUTH_TOKEN_INVALID |

---

## خدمات الطبيب — تعيين `POST /practitioners/:id/services`

> يتطلب صلاحية `practitioners:edit`

| # | الاسم | الوصف | النتيجة المتوقعة |
|---|-------|-------|-----------------|
| P1 | تعيين أساسي | serviceId + availableTypes=["clinic_visit"] | 201 + practitionerService.id |
...
| P15 | practitionerId وهمي | UUID وهمي للطبيب | 404 PRACTITIONER_NOT_FOUND |
| P16 | تكرار التعيين | تعيين نفس الخدمة مرتين | 409 ALREADY_ASSIGNED |
| P17 | طبيب يعيّن لملف طبيب آخر | practitionerId مختلف عن المستخدم الحالي | 403 FORBIDDEN |
| P18 | بدون صلاحية | مستخدم بدون practitioners:edit | 403 FORBIDDEN |

---

## خدمات الطبيب — قراءة `GET /practitioners/:id/services` (عام)
...
| P23 | طبيب وهمي | UUID غير موجود | 404 PRACTITIONER_NOT_FOUND |

---

## خدمات الطبيب — تعديل `PATCH /practitioners/:id/services/:serviceId`
...
| P32 | طبيب يعدّل ملف طبيب آخر | practitionerId مختلف عن المستخدم الحالي | 403 FORBIDDEN |

---

## خدمات الطبيب — حذف `DELETE /practitioners/:id/services/:serviceId`
...
| P36 | طبيب يحذف من ملف آخر | practitionerId مختلف عن المستخدم الحالي | 403 FORBIDDEN |
| P37 | بدون صلاحية | مستخدم بدون practitioners:edit | 403 FORBIDDEN |

---

## خدمات الطبيب — أنواع الحجز `GET /practitioners/:id/services/:serviceId/types`

> يتطلب صلاحية `practitioners:view`
...
| P41 | بدون صلاحية | مستخدم بدون practitioners:view | 403 FORBIDDEN |
| P42 | تعيين غير موجود | serviceId غير معيّن للطبيب | 404 NOT_FOUND |
| P43 | طبيب وهمي | UUID وهمي | 404 PRACTITIONER_NOT_FOUND |

---

## جدول المتاحية — `GET/PUT /practitioners/:id/availability`

> GET عام — PUT يتطلب `practitioners:edit`

| # | الاسم | الوصف | النتيجة المتوقعة |
|---|-------|-------|-----------------|
| AV1 | قراءة بدون توكن | GET /practitioners/:id/availability | 200 + مصفوفة slots أسبوعية |
...
| AV11 | بدون صلاحية | مستخدم بدون practitioners:edit | 403 FORBIDDEN |
| AV12 | بدون مصادقة | PUT بدون token | 401 AUTH_TOKEN_INVALID |
| AV13 | طبيب وهمي | UUID وهمي | 404 PRACTITIONER_NOT_FOUND |

---

## المواعيد المتاحة — `GET /practitioners/:id/slots` (عام)

| # | الاسم | الوصف | النتيجة المتوقعة |
|---|-------|-------|-----------------|
| SL1 | قراءة بدون توكن | GET /practitioners/:id/slots?date=2026-04-15 | 200 + slots[] |
...
| SL12 | طبيب وهمي | UUID وهمي | 404 PRACTITIONER_NOT_FOUND |

---

## الاستراحات — `GET/PUT /practitioners/:id/breaks`

> GET يتطلب `practitioners:view` — PUT يتطلب `practitioners:edit`

| # | الاسم | الوصف | النتيجة المتوقعة |
|---|-------|-------|-----------------|
| B1 | تعيين استراحات | PUT + breaks[] لأيام متعددة | 200 + الاستراحات محفوظة |
| B2 | قراءة الاستراحات | GET /practitioners/:id/breaks | 200 + مصفوفة |
...
| B9 | بدون صلاحية | مستخدم بدون practitioners:edit | 403 FORBIDDEN |
| B10 | بدون مصادقة | PUT بدون token | 401 AUTH_TOKEN_INVALID |
| B11 | طبيب وهمي | UUID وهمي | 404 PRACTITIONER_NOT_FOUND |

---

## الإجازات — `/practitioners/:id/vacations`

> POST + DELETE يتطلبان `practitioners:edit` — GET يتطلب `practitioners:view`
...
| V8 | قراءة الإجازات | GET /practitioners/:id/vacations | 200 + مصفوفة |
| V9 | تأثير على Slots | يوم ضمن الإجازة | 200 + قائمة slots فارغة |
| V10 | حذف ناجح | DELETE /practitioners/:id/vacations/:vacationId | 200 |
...
| V14 | بدون صلاحية | مستخدم بدون practitioners:edit | 403 FORBIDDEN |
