...
export function SessionDetailHeader({ session, onEnrollClick, onSetDateClick }: Props) {
  const { t, locale } = useLocale()
  const { cancelSessionMut } = useGroupSessionsMutations()

  const name = locale === "ar" ? session.nameAr : session.nameEn
  const practitioner = session.practitioner?.nameAr ?? ""
  ...
  const needsDate = session.schedulingMode === "on_capacity" && !session.startTime
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader title={name} description={`${practitioner} — ${dateDisplay}`}>
        {canAct && (
          <>
            {needsDate && session.currentEnrollment >= session.minParticipants && (
              <Button
                variant="outline"
      ...
      <div className="flex items-center gap-4">
        ...
        <span className="text-sm text-muted-foreground">
          {session.currentEnrollment}/{session.maxParticipants} {t("groupSessions.enrolled")}
          {confirmed > 0 && ` · ${confirmed} ${t("groupSessions.paid")}`}
          {awaiting > 0 && ` · ${awaiting} ${t("groupSessions.awaitingPayment")}`}
        </span>
        <span className="text-sm text-muted-foreground">
          {session.durationMinutes} {t("groupSessions.minutes")}
        </span>
      </div>
    </div>
  )
