...
- PT-WI9: خطأ في error code — الكود الصحيح `PATIENT_PHONE_EXISTS` وليس `CONFLICT`
...
- POST /patients/walk-in له سلوك idempotent مفقود — إذا وُجد الرقم كـ walk-in يعيد 200 + بيانات موجودة (ليس 409)
- سيناريوهات 401 و 403 مفقودة كلياً
- chronicConditions max 1000 مفقود من الوثيقة الأصلية
- GET /patients/:id يعيد آخر 10 حجوزات (ليس "آخر الحجوزات" بشكل مبهم)
- PATCH /patients/:id يحدّث User + PatientProfile في transaction واحدة
...
|---|-------|-------|-----------------|
| PT-L1 | قراءة الكل | GET /patients (بصلاحية patients:view) | 200 + { items: [...], meta: { total, page, perPage } } |
| PT-L2 | بحث بالاسم | GET /patients?search=محمد | 200 + مرضى يحتوي اسمهم على "محمد" (firstName أو lastName) |
| PT-L3 | بحث بالهاتف | GET /patients?search=+9665 | 200 + نتائج مطابقة للرقم |
| PT-L4 | بحث بالإيميل | GET /patients?search=@gmail | 200 + نتائج مطابقة |
| PT-L5 | pagination | GET /patients?page=2&perPage=10 | 200 + الصفحة الثانية (مجموعة مختلفة عن الأولى) |
| PT-L6 | مريض بـ ID | GET /patients/:id | 200 + بيانات كاملة + آخر 10 حجوزات (مع اسم الخدمة والطبيب وبيانات الدفع) |
| PT-L7 | ID وهمي | GET /patients/:uuid-غير-موجود | 404 — "Patient not found" |
| PT-L8 | إحصائيات المريض | GET /patients/:id/stats | 200 + { totalBookings, byStatus, totalPaid, completedPayments } |
| PT-L9 | بدون صلاحية patients:view | مستخدم بدون الصلاحية يطلب GET /patients | 403 FORBIDDEN |
| PT-L10 | بدون مصادقة | GET /patients بدون token | 401 AUTH_TOKEN_INVALID |

---

## إنشاء مريض Walk-in

> Endpoint: `POST /patients/walk-in` — يتطلب صلاحية `patients:create`
...
| PT-WI3 | مع بيانات طبية | bloodType=A_POS + allergies + chronicConditions | 201 + كل الحقول محفوظة في PatientProfile |
...
| PT-WI10 | phone لحساب كامل موجود | الرقم مرتبط بـ accountType=full | 409 PATIENT_PHONE_EXISTS (يتضمن userId في الرد) |
...
| PT-WI16 | بدون صلاحية patients:create | مستخدم بدون الصلاحية | 403 FORBIDDEN |
| PT-WI17 | بدون مصادقة | POST /patients/walk-in بدون token | 401 AUTH_TOKEN_INVALID |

---

## تحويل Walk-in إلى حساب كامل (Claim)

> Endpoint: `POST /patients/claim` — يتطلب صلاحية `patients:create`
...
| PT-CL10 | بدون مصادقة | POST /patients/claim بدون token | 401 AUTH_TOKEN_INVALID |

---

## تعديل بيانات المريض

> Endpoint: `PATCH /patients/:id` — يتطلب صلاحية `patients:edit`
...
| PT-U3 | تعديل البيانات الطبية | allergies + chronicConditions | 200 + PatientProfile محدَّث |
| PT-U4 | تعديل جزئي | حقل واحد فقط | 200 + فقط الحقل المُرسَل يتغير |
| PT-U5 | phone بتنسيق خاطئ | phone="0501234567" | 400 VALIDATION_ERROR |
| PT-U6 | مريض غير موجود | ID وهمي | 404 — "Patient not found" |
| PT-U7 | firstName يتجاوز 255 حرف | 256 حرف | 400 VALIDATION_ERROR |
| PT-U8 | nationalId يتجاوز 20 حرف | 21 حرف | 400 VALIDATION_ERROR |
| PT-U9 | بدون صلاحية patients:edit | مستخدم بدون الصلاحية | 403 FORBIDDEN |
| PT-U10 | بدون مصادقة | PATCH /patients/:id بدون token | 401 AUTH_TOKEN_INVALID |
