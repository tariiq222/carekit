...
export class XmlBuilderService {
  ...
  private addHeader(root: ReturnType<typeof create>, input: ZatcaXmlInput): void {
    ...
    root
      ...
      .ele('cbc:UUID').txt(input.invoiceNumber).up()
      .up();
    root
      .ele('cac:AdditionalDocumentReference')
  ...
  private addBuyer(root: ReturnType<typeof create>, buyer: { name: string; vatNumber?: string }): void {
    const party = root.ele('cac:AccountingCustomerParty').ele('cac:Party');
    party.ele('cac:PartyLegalEntity').ele('cbc:RegistrationName').txt(buyer.name).up().up();
    if (buyer.vatNumber) {
      party.ele('cac:PartyTaxScheme').ele('cbc:CompanyID').txt(buyer.vatNumber).up().ele('cac:TaxScheme').ele('cbc:ID').txt('VAT').up().up().up();
    }
    root.up();
  }
  ...
  private addLegalMonetaryTotal(root: ReturnType<typeof create>, input: ZatcaXmlInput): void {
    ...
    root
      ...
      .ele('cbc:LineExtensionAmount', { currencyID: 'SAR' }).txt(excl).up()
      .ele('cbc:TaxExclusiveAmount', { currencyID: 'SAR' }).txt(excl).up()
      .ele('cbc:TaxInclusiveAmount', { currencyID: 'SAR' }).txt(incl).up()
      .ele('cbc:AllowanceTotalAmount', { currencyID: 'SAR' }).txt('0.00').up()
      .ele('cbc:PrepaidAmount', { currencyID: 'SAR' }).txt('0.00').up()
      .ele('cbc:PayableAmount', { currencyID: 'SAR' }).txt(incl).up()
      .up();
  }
}
