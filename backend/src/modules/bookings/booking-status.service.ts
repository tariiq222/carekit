        ...
        if (booking.status !== 'checked_in') {
          if (booking.status === 'confirmed') {
            throw new BadRequestException({
              statusCode: 400,
              message:
                'يجب تسجيل وصول المريض أولاً — استخدم checkIn قبل startSession',
        ...
        if (booking.status !== 'in_progress') {
          if (booking.status === 'checked_in') {
            throw new BadRequestException({
              statusCode: 400,
              message:
                'يجب بدء الجلسة أولاً قبل إتمام الحجز — استخدم startSession أولاً',
          ...
          if (booking.status === 'confirmed') {
            throw new BadRequestException({
              statusCode: 400,
              message:
                'يجب تسجيل وصول المريض وبدء الجلسة أولاً قبل إتمام الحجز',
  ...
  async recoverExpiredBooking(bookingId: string): Promise<boolean> {
    ...
    if (result.count > 0) {
      this.statusLogService
        ...
        .catch((err) =>
          this.logger.warn('Status log failed on recovery', {
