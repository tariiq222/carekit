...
describe('Branches Filters (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const res = await request(httpServer)
      ...
      .send({ nameAr: 'فرع جدة المميز', nameEn: 'Jeddah Distinct Branch', address: 'طريق الملك عبدالعزيز', isActive: false })
      .expect(201);
  ...
  it('should filter by search term matching nameAr', async () => {
    const res = await request(httpServer)
      .get(`${URL}?search=جدة المميز`).set(getAuthHeaders(superAdmin.accessToken)).expect(200);
    expectSuccessResponse(res.body);
    const items = (res.body.data?.items ?? res.body.data) as Array<{ nameAr: string; nameEn: string }>;
    expect(items.some((b) => b.nameAr.includes('جدة'))).toBe(true);
  });

  it('should filter by search term matching nameEn', async () => {
    const res = await request(httpServer)
      .get(`${URL}?search=Jeddah Distinct`).set(getAuthHeaders(superAdmin.accessToken)).expect(200);
    expectSuccessResponse(res.body);
    const items = (res.body.data?.items ?? res.body.data) as Array<{ id: string }>;
  ...
  it('should return only inactive branches with isActive=false', async () => {
    ...
    expectSuccessResponse(res.body);
    const items = (res.body.data?.items ?? res.body.data) as Array<{ isActive: boolean }>;
  ...
  it('should return only active branches with isActive=true', async () => {
    const res = await request(httpServer)
      .get(`${URL}?isActive=true`).set(getAuthHeaders(superAdmin.accessToken)).expect(200);
    expectSuccessResponse(res.body);
    const items = (res.body.data?.items ?? res.body.data) as Array<{ isActive: boolean }>;
    expect(items.length).toBeGreaterThan(0);
  ...
  it('should paginate with page=1&perPage=1 returning one item and correct meta', async () => {
    const res = await request(httpServer)
      .get(`${URL}?page=1&perPage=1`).set(getAuthHeaders(superAdmin.accessToken)).expect(200);
    expectSuccessResponse(res.body);
    const data = res.body.data as { items?: unknown[]; meta?: { page: number; perPage: number } };
    if (data.items) {
