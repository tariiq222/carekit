...
describe('IntakeFormsService — extended coverage', () => {
  ...
  describe('listForms', () => {
    it('should return all forms without filters', async () => {
      const forms = [{ id: 'form-1', scope: 'global' }, { id: 'form-2', scope: 'service' }];
    ...
    it('should filter by scope when provided', async () => {
      ...
      await service.listForms({ scope: 'global' } as Parameters<typeof service.listForms>[0]);
    ...
    it('should filter by isActive=false when provided', async () => {
      ...
      await service.listForms({ isActive: false } as Parameters<typeof service.listForms>[0]);
    ...
    it('should filter by serviceId when provided', async () => {
      ...
      await service.listForms({ serviceId: 'svc-1' } as Parameters<typeof service.listForms>[0]);

      expect(mockPrisma.intakeForm.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ serviceId: 'svc-1' }) }),
      );
    });
    ...
    it('should not include isActive in where when isActive is undefined', async () => {
      ...
      const call = mockPrisma.intakeForm.findMany.mock.calls[0][0] as { where: Record<string, unknown> };
    ...
    it('should filter by practitionerId when provided', async () => {
      ...
      await service.listForms({ practitionerId: 'prac-1' } as Parameters<typeof service.listForms>[0]);

      expect(mockPrisma.intakeForm.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ practitionerId: 'prac-1' }) }),
      );
    });
    ...
    it('should filter by branchId when provided', async () => {
      ...
      await service.listForms({ branchId: 'branch-1' } as Parameters<typeof service.listForms>[0]);
  ...
  describe('setFields — options, condition, sortOrder', () => {
    ...
    it('should store options array for select fields', async () => {
      ...
      await service.setFields('form-1', {
        fields: [{ labelAr: 'تخصص', labelEn: 'Specialty', fieldType: 'select', options: ['A', 'B', 'C'] }],
      } as Parameters<typeof service.setFields>[1]);

      expect(mockTx.intakeField.createMany).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.arrayContaining([expect.objectContaining({ options: ['A', 'B', 'C'] })]),
        }),
      );
    });

    it('should store condition object for conditional fields', async () => {
      const condition = { fieldId: 'field-ref', operator: 'equals', value: 'yes' };
      mockPrisma.intakeForm.findUnique.mockResolvedValue({ id: 'form-1' });
      mockPrisma.$transaction.mockImplementation((fn: (tx: typeof mockTx) => Promise<unknown>) => fn(mockTx));
      mockTx.intakeField.deleteMany.mockResolvedValue({ count: 0 });
      mockTx.intakeField.createMany.mockResolvedValue({ count: 1 });
      ...
      await service.setFields('form-1', {
        fields: [{ labelAr: 'تفاصيل', labelEn: 'Details', fieldType: 'textarea', condition }],
      } as Parameters<typeof service.setFields>[1]);

      expect(mockTx.intakeField.createMany).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.arrayContaining([expect.objectContaining({ condition })]),
        }),
      );
    });
    ...
    it('should use provided sortOrder when explicitly set', async () => {
      ...
      await service.setFields('form-1', {
        fields: [{ labelAr: 'ثالث', labelEn: 'Third', fieldType: 'text', sortOrder: 5 }],
      } as Parameters<typeof service.setFields>[1]);

      expect(mockTx.intakeField.createMany).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.arrayContaining([expect.objectContaining({ sortOrder: 5 })]),
        }),
      );
    });
  });
  ...
  describe('submitResponse — return value', () => {
    it('should return only the response (not the form update)', async () => {
      const mockResponse = { id: 'resp-1', formId: 'form-1', patientId: 'p-1', answers: {} };
      mockPrisma.intakeForm.findUnique.mockResolvedValue({ id: 'form-1' });
      mockPrisma.booking.findUnique.mockResolvedValue({ patientId: 'p-1' });
      mockPrisma.$transaction.mockResolvedValue([mockResponse, { id: 'form-1', submissionsCount: 2 }]);

      const result = await service.submitResponse('p-1', { formId: 'form-1', bookingId: 'booking-1', answers: {} });
    ...
    it('should use $transaction for atomic create + counter increment', async () => {
      ...
      await service.submitResponse('p-1', { formId: 'form-1', bookingId: 'booking-1', answers: {} });
  ...
  describe('validateScopeTarget — skip when FK not provided', () => {
    it('should skip service lookup when scope=service but serviceId is undefined', async () => {
      const dto = { scope: 'service', nameAr: 'ن', nameEn: 'N', type: 'pre_booking' } as Parameters<typeof service.createForm>[0];
      mockPrisma.intakeForm.create.mockResolvedValue({ id: 'f1', ...dto, isActive: true, fields: [] });
    ...
    it('should skip practitioner lookup when scope=practitioner but practitionerId is undefined', async () => {
      const dto = { scope: 'practitioner', nameAr: 'ن', nameEn: 'N', type: 'pre_booking' } as Parameters<typeof service.createForm>[0];
      mockPrisma.intakeForm.create.mockResolvedValue({ id: 'f1', ...dto, isActive: true, fields: [] });
    ...
    it('should skip branch lookup when scope=branch but branchId is undefined', async () => {
      const dto = { scope: 'branch', nameAr: 'ن', nameEn: 'N', type: 'pre_booking' } as Parameters<typeof service.createForm>[0];
      mockPrisma.intakeForm.create.mockResolvedValue({ id: 'f1', ...dto, isActive: true, fields: [] });
