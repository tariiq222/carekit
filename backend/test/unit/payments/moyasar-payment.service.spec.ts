...
import { MoyasarCheckoutService } from '../../../src/modules/payments/moyasar-checkout.service.js';
...
const mockGroupsPaymentService: any = {
...
const mockConfigService: any = {
  get: jest.fn(
    (key: string, fallback?: string) => configMap[key] ?? fallback ?? '',
  ),
};
...
describe('MoyasarPaymentService', () => {
  ...
  describe('createMoyasarPayment', () => {
    ...
    it('should throw NotFoundException when booking not found', async () => {
      ...
      await expect(
        service.createMoyasarPayment(userId, createDto),
    ...
    it('should throw BadRequestException when Moyasar API returns error', async () => {
      ...
      mockFetch.mockResolvedValue({
        ok: false,
        json: async () => ({ message: 'Invalid card' }),
      });
      await expect(
        service.createMoyasarPayment(userId, createDto),
      ).rejects.toThrow(BadRequestException);
    ...
    it('should handle no transaction_url from Moyasar', async () => {
      ...
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => ({ id: moyasarPayId, status: 'paid', source: {} }),
      });
      const result = await service.createMoyasarPayment(userId, createDto);
  ...
  describe('handleMoyasarWebhook', () => {
    ...
    it('should not fail if invoice creation throws ConflictException', async () => {
      ...
      mockPrisma.processedWebhook.create.mockResolvedValue({});
      mockBookingStatusService.confirm.mockResolvedValue({});
      mockInvoicesService.createInvoice.mockRejectedValue(
  ...
  describe('refund', () => {
    ...
    it('should call Moyasar refund API and update status to refunded', async () => {
      ...
      mockPrisma.payment.updateMany.mockResolvedValue({ count: 1 });
      mockPrisma.payment.findUniqueOrThrow = jest
        .fn()
        .mockResolvedValue(refundedPayment);
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => ({ id: moyasarPayId }),
      });
    ...
    it('should throw NotFoundException when payment not found', async () => {
      mockPrisma.payment.findUnique.mockResolvedValue(null);
      await expect(service.refund('non-existent')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw BadRequestException when payment is not paid', async () => {
      ...
      await expect(service.refund(paymentId)).rejects.toThrow(
        BadRequestException,
      );
    });
